...
if st.session_state.authenticated:
                  ...
                  with st.sidebar:
                          selected = option_menu(
                              ...
                              icons=["house","gear","bell fill","ethernet","motherboard","text-paragraph","braces asterisk","pin","rss"],
                              menu_icon="cast",
                              default_index=0,
                          )
                  ...
                  if selected =="Config Review":
                     ...
                     # Create a button to show the description dialog box
                      ...
                      modal = Modal(key="mitigation",title="Mitigation")
                      open_modal = st.button(label='Mitigation')
                      if open_modal:
                         with modal.container():
                             st.markdown("set the following UI path to Enabled Computer ")
                             st.markdown("Configuration\Policies\Windows Settings\Security Settings\Local Policies\Security Options\Microsoft network client: Digitally sign communications (always)")
                  ...
                  if selected == "Risk Score":
                          ...
                          with col1:
                              ...
                              st.write("The Common Vulnerability Scoring System (CVSS) provides a way to capture the principal characteristics of a 
vulnerability and produce a numerical score reflecting its severity. 
The numerical score can then be translated into a qualitative representation (such as low, medium, high, and critical) to help organizations properly assess 
and prioritize their vulnerability management processes.CVSS is a published standard used by organizations worldwide, and the SIG's mission is to continue to improve it.")
                              st.title("39.67")
                          with col2:
                              st.dataframe(df)
